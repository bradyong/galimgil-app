...
function inferCategory(question, choiceA, choiceB, profile) {
  ...
  if (includesAny(text, ["매수", "매도", "투자", "코인", "주식", "적금", "예금", "종목", "차트", "팔까", "살까말까"])) return "money";
  if (includesAny(text, ["소비", "살까", "구매", "쇼핑", "예약", "결제", "주문", "장바구니", "바꿀까", "바꾼", "바꾼다", "바꾸", "교체", "버틴", "버티", "중고", "새거", "새것", "새 걸", "새걸", "컴퓨터", "pc", "피씨", "노트북", "게임기", "플스", "플레이스테이션", "닌텐도", "스위치", "자동차", "차량", "차 산", "차 살", "차 바꿀", "차 구매", "차 계약", "휴대폰", "핸드폰", "스마트폰", "폰", "아이폰", "갤럭시", "에어컨", "tv", "티비", "텔레비전", "가전"])) return "shopping";
  return "daily";
}
...
function subjectAnchorLine(subjectProfile, winner, loser, question, seed) {
  ...
  if (subject === "여행지") {
    const lines = [
      ...
      `${winnerName} 쪽은 가서 뭘 할지보다 가는 길과 돌아오는 길까지 같이 봐야 합니다. 오늘은 그 동선이 조금 더 편하게 이어집니다.`,
      `여행지는 결국 도착 전 설렘과 돌아온 뒤 피로가 같이 심사합니다. ${winnerName} 쪽이 오늘은 그 균형을 조금 더 잘 맞춥니다.`
    ];
    return cleanPlayTone(pick(lines, hashText(`${question}-${winner.name}-${loser.name}-${subject}-${seed}-travel-anchor`)));
  }
  if (subject === "놀이 선택") {
    const lines = [
      `이건 장소 이름보다 게임, 노래, 시간 사용이 갈리는 선택입니다. 오늘은 ${winnerName} 쪽이 놀고 난 뒤 표정이 조금 더 편해 보입니다.`,
      `${winnerName} 쪽은 시작하는 순간의 재미와 끝낼 타이밍을 같이 봐야 합니다. ${loserName}도 좋지만 오늘은 이쪽 리듬이 더 살아 있습니다.`,
      `놀이 선택은 재밌는지만 보면 안 됩니다. 시간이 어떻게 사라지는지까지 보면 ${winnerName} 쪽이 오늘 더 말이 됩니다.`
    ];
    return cleanPlayTone(pick(lines, hashText(`${question}-${winner.name}-${loser.name}-${subject}-${seed}-hobby-anchor`)));
  }
  const lines = [
...
function ensureSubjectInReason(reason, subjectProfile, winner, loser, question, seed) {
  ...
  if (subjectProfile.confidence < 0.7) return reason;
...
function analyzeQuestion(question, choiceA, choiceB, profile = {}) {
  ...
  const moneyWords = ["적금", "예금", "주식", "코인", "투자", "매수", "매도", "종목", "차트", "팔까", "판다", "안판다", "보유", "홀딩", "관망"];
  if (includesAny(compact, moneyWords)) {
    const a = semanticOptionTraits(optionA, "money", "investment_compare");
    const b = semanticOptionTraits(optionB, "money", "investment_compare");
    return {
      optionA,
      optionB,
      category: "money",
      subCategory: "investment_compare",
      intent: "돈을 어디에 두거나 어떻게 움직일지 고민",
      optionA_traits: a.traits,
      optionB_traits: b.traits,
      optionA_vibe: a.vibe,
      optionB_vibe: b.vibe,
      subjectProfile: extractSubjectProfile(question, optionA, optionB, "money"),
      confidence: 0.9
    };
  }
  ...
  if (includesAny(compact, shoppingCompareWords)) {
    ...
    const b = semanticOptionTraits(optionB, "shopping", "purchase_compare");
    return {
      optionA,
      optionB,
      category: "shopping",
      subCategory: "purchase_compare",
      intent: "가격, 사용 빈도, 후회가 걸린 구매 고민",
      optionA_traits: a.traits,
      optionB_traits: b.traits,
      optionA_vibe: a.vibe,
      optionB_vibe: b.vibe,
...
function optionSceneVariant(category, winner, loser, question, sign, seed) {
  ...
  if (category === "travel") {
    if (includesAny(winnerRaw, ["일본"])) pools.push("일본은 회사 생각을 잊는다기보다, 걷고 구경하느라 생각할 틈을 줄이는 쪽입니다.", "편의점 한 바퀴만 돌아도 괜히 여행 온 티가 나는 선택입니다.", "많이 걷는 건 각오해야 하지만, 골목마다 오늘의 핑계가 생깁니다.", "일본은 아침부터 밤까지 동선을 채우는 쪽입니다. 발은 힘든데 눈은 계속 바쁩니다.", "면세점보다 편의점에서 먼저 여행 온 기분이 날 수 있습니다.");
    if (includesAny(winnerRaw, ["태국"])) pools.push("태국은 회사 생각이 덜 나는 쪽입니다. 슬리퍼 신고 느슨해지는 그림이 먼저 옵니다.", "마사지 받고 야시장 걷는 순간 휴가가 빨리 시작됩니다.", "계획표보다 몸이 먼저 풀리는 여행에 가깝습니다.", "태국은 일정을 꽉 채우기보다 더운 공기 안에서 속도를 늦추는 쪽입니다.", "수영장과 망고 앞에서는 평소 걱정이 잠깐 목소리를 낮춥니다.");
    if (includesAny(winnerRaw, ["제주"])) pools.push("제주는 차 타고 창밖 보는 시간까지 여행처럼 느껴지는 쪽입니다.", "카페, 바람, 드라이브가 한 세트로 따라옵니다.", "오름 하나만 넣어도 하루가 갑자기 여행처럼 변합니다.", "제주는 이동 중 창밖이 계속 말을 거는 쪽입니다.", "날씨만 맞으면 별 계획 없어도 바람이 알아서 한몫합니다.");
    if (includesAny(winnerRaw, ["부산"])) pools.push("부산은 바다 보고 회 한 점 먹는 순간 말이 쉬워집니다.", "밤바다 앞에서는 피곤해도 괜히 한 번 더 걷게 됩니다.", "도착하면 여행보다 놀러 온 느낌이 먼저 납니다.", "부산은 밥 먹고 바다 보고 또 뭘 먹을지 생각나는 쪽입니다.", "기차에서 내리면 이미 바람 냄새가 약속을 시작합니다.");
    if (includesAny(winnerRaw, ["강릉"])) pools.push("강릉은 바다 보고 커피까지 이어지는 동선이 쉬운 쪽입니다.", "해변에서 한 번 걷고 카페에 앉으면 하루가 꽤 그럴듯해집니다.", "강릉은 바닷바람보다 커피잔이 먼저 손에 잡히는 여행일 수 있습니다.", "주차만 잘 풀리면 밥, 바다, 카페가 순서대로 따라옵니다.", "강릉은 크게 무리하지 않아도 여행 온 척이 잘 되는 쪽입니다.");
    if (includesAny(winnerRaw, ["양양"])) pools.push("양양은 강릉보다 조금 더 바람과 서핑 쪽으로 마음이 갑니다.", "양양은 카페보다 해변에 오래 서 있고 싶은 날에 잘 맞습니다.", "양양은 계획표보다 바다 앞에서 멍 때리는 시간이 본편입니다.", "사람 많은 코스보다 조금 더 느슨한 바다 쪽이면 양양이 말을 겁니다.", "양양은 돌아다니기보다 한 장소에 오래 붙어 있어도 덜 아까운 쪽입니다.");
    if (includesAny(winnerRaw, ["삼척"])) pools.push("삼척은 유명 코스보다 조용한 바다와 드라이브가 먼저 떠오릅니다.", "삼척은 사람이 적은 길에서 숨 돌리는 맛이 있습니다.", "바다를 보러 갔는데 마음까지 조금 조용해지는 쪽입니다.", "삼척은 카페 줄보다 해안길 쪽에 시간을 쓰는 여행입니다.", "삼척은 크게 떠들지 않아도 다녀온 느낌이 남는 쪽입니다.");
    if (!pools.length) pools.push(`${winnerName}은 도착해서 뭘 할지보다 가는 길의 리듬이 더 잘 맞습니다.`, `${winnerName} 고르면 일정표가 빡빡하지 않아도 하루가 덜 납작합니다.`, `${loserName}도 좋지만, 오늘은 ${winnerName} 쪽 동선이 덜 억지스럽습니다.`, `${winnerName}은 비용보다 다녀온 뒤 몸이 얼마나 덜 투덜댈지가 먼저 보입니다.`, `${winnerName} 쪽은 가서 먹고 걷고 쉬는 순서가 조금 더 자연스럽습니다.`, `${winnerName}은 멀리 간 보람보다 오늘 기분 전환을 빨리 만드는 쪽입니다.`);
  }
  if (category === "shopping") {
...
function foodRealityReason(winner, loser, question, sign, seed = 0) {
  ...
  if (snackLikeName(winner.name) || snackLikeName(loser.name)) {
    ...
    const lines = [
      ...
      `${withParticle(winner.name, "을", "를")} 먼저 해석하면 ${w0}에 ${w1}이 붙는 간식입니다. ${withParticle(loser.name, "은", "는")} ${withParticle(l0, "과", "와")} ${l1} 쪽이고요. 오늘은 배보다 손이 고르는 문제라 ${winnerName} 쪽입니다.`,
...
function sameResultDifferentReason(category, winner, loser, question, cards = [], sign = ["황소자리", "♉"], seed = 0) {
  ...
  if (category === "travel") {
    if (hasRaw(["바다", "계곡", "해변", "물가", "해수욕장"])) {
      ...
      const natureScenes = seaWin ? [
        ...
        `${winnerSubject} 낮에도 좋고 밤바다로 이어져도 말이 됩니다. ${loserSubject} 차분하지만 오늘 사진첩은 바다 쪽에 더 오래 멈춥니다.`,
        ...
        `${winnerSubject} 도착하면 발부터 물에 넣고 싶어지는 선택입니다. ${loserSubject} 넓게 트이지만, 오늘은 그늘 아래 오래 앉는 쪽이 이깁니다.`,
        `계곡은 사진보다 온도가 먼저 기억납니다. 차가운 물, 그늘, 젖은 슬리퍼까지 생각하면 ${winnerName} 쪽이 더 쉬는 날 같습니다.`,
        `${loserName}도 끌리지만 오늘은 물소리 옆에서 말수가 줄어드는 쪽입니다. ${winnerSubject} 하루를 크게 벌이지 않고도 쉬는 티가 납니다.`
      ] : [
        ...
        `${winnerSubject} 도착하자마자 공기가 바뀌는 선택입니다. ${loserName}도 후보지만 오늘은 물소리나 바람이 있는 쪽입니다.`
      ];
    ...
    const f0 = escapeHtml(f0Raw);
    const f0Particle = withParticle(f0Raw, "이", "가");
    ...
    const travelScenes = [
      ...
      `${winnerName}은 첫 목적지보다 그다음 발걸음이 덜 막힙니다. 밥 먹고 걷고 쉬는 순서가 자연스러우면 여행 피로가 덜 투덜댑니다.`,
      `오늘은 "가서 뭘 남기지?"보다 "가서 얼마나 편하게 움직이지?"가 먼저입니다. ${winnerName} 쪽은 ${f0Particle} 붙어서 시작이 덜 뻣뻣합니다.`,
      `${loserName}에도 ${l1}도 있긴 한데, 오늘은 이동 중에 짜증이 덜 날 쪽이 더 큽니다. ${winnerName}은 하루 리듬이 조금 더 매끈합니다.`,
      `${winnerName}은 이름을 듣자마자 하루의 온도가 잡힙니다. ${winnerVibe} 쪽으로 마음이 기울고, ${loserName}은 다음 여행 후보석에 앉습니다.`,
      `먹거리 기준으로 보면 ${winnerName} 쪽이 중간에 멈춰 앉을 핑계가 더 잘 생깁니다. 여행은 구경만 하는 게 아니라 배도 설득해야 합니다.`,
      `${winnerName} 쪽은 도착한 뒤 첫 두 시간이 빨리 그려집니다. ${loserName}도 좋은데, 오늘은 움직이는 동선이 더 자연스러운 쪽입니다.`,
      `오늘 여행은 거창한 추억보다 돌아오는 길 표정이 중요합니다. ${winnerName}은 피곤해도 "그래도 괜찮았다"가 나올 여지가 더 있습니다.`,
      `지금 필요한 건 여행지 이름 자체보다 하루가 어떻게 흘러갈지입니다. ${winnerName}은 ${f0}부터 ${f1}까지 이어지는 느낌이 더 또렷합니다.`,
      `${loserTopic} 조용히 손을 들고 있지만, ${winnerTopic} 벌써 지도 앱에서 핀을 꽂고 있습니다. 오늘은 바로 움직일 수 있는 쪽입니다.`,
      `${winnerName}은 길 위에서 시간을 덜 버릴 것 같은 선택입니다. 목적지보다 중간중간 쉬는 타이밍이 더 잘 맞아 보입니다.`,
      `오늘은 유명한 곳을 찍는 것보다 내 체력이 덜 삐지는 쪽이 이깁니다. ${winnerName}은 그 기준에서 조금 더 현실적입니다.`,
      `${winnerName} 쪽은 "가면 뭐 먹지?"가 바로 붙습니다. 여행지는 결국 발보다 배가 먼저 기억할 때도 있습니다.`,
      `${winnerName}은 하루를 크게 벌이지 않아도 여행 온 느낌을 만들 수 있습니다. ${loserName}은 좋은데 오늘은 준비 에너지가 조금 더 필요해 보입니다.`,
      `이번 선택은 멀리 가는 폼보다 다녀온 뒤 몸이 얼마나 덜 파업하느냐입니다. ${winnerName} 쪽이 그 부분에서 한 끗 앞섭니다.`
    ];
  ...
  if (category === "exercise") {
    const exercisePool = [
      `${winnerName}은 시작 전엔 귀찮아도 끝나고 몸이 먼저 대답하는 쪽입니다. ${loserName}도 괜찮지만 오늘은 땀보다 리듬을 살리는 선택입니다.`,
      `${winnerName} 쪽은 운동이라는 말보다 끝나고 숨이 어떻게 돌아오는지가 먼저 보입니다. ${loserTopic} 강도가 다르지만 오늘은 몸이 받아줄 속도가 더 중요합니다.`,
      `공원산책과 런닝은 같은 운동처럼 보여도 결이 다릅니다. 오늘은 기록보다 몸을 풀고 돌아오는 ${winnerName} 쪽이 덜 부담스럽습니다.`,
      `${winnerName}은 끝나고 "그래도 움직였다"가 나오는 선택입니다. ${loserName}은 더 강하게 밀어붙이지만 오늘 몸은 이쪽 속도를 더 잘 받을 것 같습니다.`,
      `운동화 끈을 묶는 순간부터 답이 갈립니다. 오늘은 무리해서 증명하기보다 ${winnerName}처럼 끝까지 할 수 있는 쪽입니다.`
    ];
    return cleanPlayTone(pick(exercisePool, hashText(`${question}-${winner.name}-${loser.name}-${signName}-${seed}-exercise-varied`)));
  }
  if (category === "food") {
...
function playfulRealityReason(category, winner, loser, question) {
  ...
  const loserTopic = withParticle(loser.name, "은", "는");
  const winnerSubject = withParticle(winner.name, "은", "는");
  const loserSubjectName = withParticle(loser.name, "은", "는");
  ...
  const pools = {
    ...
    place: [
      `${winnerSubject} 나가서 뭐라도 생기는 쪽입니다. ${loserSubjectName} 체력과 지갑이 덜 삐지는 쪽이고요.`,
...
function compactResultReason(text, maxSentences = 2) {
  const cleaned = cleanPlayTone(text || "");
  const sentences = cleaned.match(/[^.!?。！？]+[.!?。！？]?/g) || [cleaned];
  return cleanPlayTone(sentences.slice(0, maxSentences).join(" "));
}

function memorableFutureCommentPool(category, winner, question, signName) {
  const name = escapeHtml(winner.name);
  const subject = escapeHtml(winner.subjectName || winner.name);
  const raw = `${question} ${winner.name} ${winner.subjectName || ""}`.replace(/\s/g, "").toLowerCase();
  const byCategory = {
    food: [
      `먹고 나서 "이걸 왜 고민했지"가 나올 수 있습니다.`,
      `배는 이미 알고 있었고 머리만 늦게 따라왔습니다.`,
      `한 번 맛보면 반대 선택은 잠깐 조용해질 예정입니다.`,
      `먹는 동안만큼은 인생 고민이 잠깐 주방 밖으로 나갑니다.`,
      `후회는 모르겠고 일단 입은 바쁠 예정입니다.`,
      `다 먹고 나서 괜히 메뉴판을 다시 안 봐도 됩니다.`,
      `오늘의 위장은 ${name} 쪽으로 결재 도장을 찍었습니다.`,
      `먹고 나면 "다음엔 저것도"라는 욕심만 남을 수 있습니다.`,
      `냅킨이 바빠지면 오늘 선택은 꽤 성공입니다.`,
      `배부른 뒤에도 생각나면 그건 그냥 진 겁니다.`,
      `반대 메뉴는 다음 생이 아니라 다음 끼니에 만나면 됩니다.`,
      `먹는 순간 말수가 줄면 설명은 끝입니다.`,
      `오늘 메뉴는 논리보다 침샘이 먼저 통과시켰습니다.`,
      `식탁 앞에서 고민이 너무 오래 버티진 못할 겁니다.`,
      `다 먹고 물 한 잔 마실 때쯤 결과가 납득될 수 있습니다.`
    ],
    beverage: [
      `컵 잡는 순간 오늘 텐션이 조금 정리됩니다.`,
      `빨대든 머그컵이든 손에 잡히면 고민은 작아집니다.`,
      `${name} 한 모금 뒤에 표정이 먼저 대답할 수 있습니다.`,
      `오늘 카페인 회의는 여기서 끝날 가능성이 큽니다.`,
      `얼음 소리나 따뜻한 컵 중 하나가 오늘을 설득합니다.`,
      `마시고 나면 "그래, 이 정도 위로면 됐다"가 나올 수 있습니다.`,
      `컵홀더가 오늘 선택의 증거로 남습니다.`,
      `첫 모금 뒤에 폰 보는 표정이 조금 달라질 예정입니다.`,
      `오늘은 음료가 아니라 내 기분에 빨대를 꽂는 선택입니다.`,
      `카페 나오는 길에 손이 덜 허전하면 성공입니다.`
    ],
    drink: [
      `잔이 돌기 시작하면 말도 같이 풀릴 수 있습니다.`,
      `내일 아침은 아직 서명을 안 했지만 오늘 밤은 고개를 끄덕입니다.`,
      `${name} 고르면 분위기는 빨라지고 귀가 시간은 협상합니다.`,
      `한 잔만 한다는 말은 오늘도 면접을 봅니다.`,
      `안주는 핑계고 사실 오늘은 기분이 먼저 왔습니다.`,
      `잔 내려놓는 순간 "여기서 멈출까?"가 새 갈림길로 옵니다.`,
      `오늘 밤의 나는 웃고, 내일의 나는 물을 찾을 수 있습니다.`,
      `술자리는 시작보다 마무리가 진짜 실력입니다.`,
      `폼 잡는 술보다 대화가 풀리는 술이 오늘은 강할 수 있습니다.`,
      `계산서보다 무서운 건 다음 잔을 부르는 분위기입니다.`
    ],
    shopping: [
      `결제 버튼은 반짝이고 통장은 눈을 피합니다.`,
      `택배 알림이 오면 미래의 내가 바로 태세 전환할 수 있습니다.`,
      `${name} 고르면 설렘은 빠르고 합리화는 더 빠릅니다.`,
      `장바구니가 드디어 구경꾼에서 선수로 뛰려 합니다.`,
      `사고 나서 자주 쓰면 승리, 안 쓰면 인테리어입니다.`,
      `오늘의 지름신은 말이 많고 통장은 짧게 대답합니다.`,
      `배송조회하는 손가락이 벌써 준비운동 중입니다.`,
      `결제 후 웃으면 성공이고, 한숨부터 나오면 재검토입니다.`,
      `사흘 뒤에도 갖고 싶으면 그때도 꽤 진심입니다.`,
      `오늘은 물건보다 "이걸 가진 나"를 산 걸 수도 있습니다.`,
      `통장은 울 수 있지만 스트레스가 조용해질 수도 있습니다.`,
      `안 사면 잔고가 살고, 사면 상상 속 내가 살아납니다.`
    ],
    relationship: [
      `보내는 건 1초, 핸드폰 보는 시간은 별도입니다.`,
      `말을 밖으로 꺼내면 마음속 소음이 조금 줄 수 있습니다.`,
      `답장은 상대 몫이고, 내 용기는 이미 출근했습니다.`,
      `혼자 상상하던 드라마가 오늘은 편성 취소될 수 있습니다.`,
      `${name} 쪽으로 가면 적어도 마음속 미리보기는 줄어듭니다.`,
      `말 안 하면 편한 척은 되지만 밤에 다시 재생될 수 있습니다.`,
      `연락은 짧고 해석은 길어서, 오늘은 짧은 쪽을 잡아야 합니다.`,
      `상대 표정은 몰라도 내 마음 표정은 조금 덜 굳을 수 있습니다.`,
      `망설임이 길어지면 대화보다 상상력이 먼저 과로합니다.`,
      `오늘 한마디가 내일의 혼잣말을 줄여줄 수 있습니다.`,
      `읽음 표시는 무섭지만 혼자 결론 내리는 것도 만만치 않습니다.`,
      `마음이 계속 문 앞에 서 있으면 한번은 열어봐야 합니다.`
    ],
    travel: [
      `돌아오는 길 피곤함보다 꺼내 볼 장면이 더 크면 이긴 겁니다.`,
      `${name} 쪽은 신발이 고생하고 사진첩이 웃을 수 있습니다.`,
      `가방 싸는 귀찮음을 지나면 하루가 조금 덜 복사본 같습니다.`,
      `도착하고 나서 "그래도 나오길 잘했네"가 나오면 성공입니다.`,
      `지도 앱 켜는 순간 이미 일상 밖으로 한 발 나갔습니다.`,
      `차창 밖 풍경이 바뀌면 생각도 조금 자리를 옮깁니다.`,
      `커피 한 잔, 걷는 길 하나만 남아도 오늘은 본전입니다.`,
      `돌아오면 다리는 피곤하고 얘깃거리는 조금 늘어납니다.`,
      `오늘 여행은 멀리보다 돌아와서 자랑할 말이 있느냐입니다.`,
      `계획표가 완벽하지 않아도 밖 공기가 대신 변명해줍니다.`,
      `숙소보다 먼저 떠오르는 장면이 있으면 그쪽이 이긴 겁니다.`,
      `차비는 빠져나가도 하루가 덜 밋밋해질 수 있습니다.`
    ],
    place: [
      `신발 신는 순간 귀찮음은 졌고 하루는 조금 움직입니다.`,
      `${name} 쪽으로 가면 집에 없던 장면 하나는 생깁니다.`,
      `나가면 피곤하고 안 나가면 아쉬운 날, 오늘은 아쉬움을 줄입니다.`,
      `도착해서 앉는 순간 방금 전 고민은 조금 과장처럼 보일 수 있습니다.`,
      `돌아오는 길 말수가 줄어도 "그래도 갔다"는 남습니다.`,
      `밖 공기가 오늘 머릿속을 살짝 환기할 수 있습니다.`,
      `엘리베이터 버튼 누르면 이미 반은 결정난 겁니다.`,
      `오늘 동선은 귀찮지만 기분에는 작은 구멍을 냅니다.`,
      `몸은 움직이고, 하루는 평소보다 덜 정지화면이 됩니다.`,
      `집에 와서 신발 벗을 때쯤 답이 다시 보일 수 있습니다.`
    ],
    game: [
      `한 판만 한다는 말이 또 신뢰도 테스트를 받습니다.`,
      `${name} 켜면 시계가 갑자기 배신자처럼 행동합니다.`,
      `재미는 바로 오고 종료 버튼은 조금 멀어질 수 있습니다.`,
      `끝나고도 머릿속 미니맵이 꺼지지 않을 가능성이 있습니다.`,
      `오늘 스트레스는 줄고 플레이타임은 늘어날 수 있습니다.`,
      `컨트롤러나 키보드가 "드디어?" 하고 일어납니다.`,
      `이기면 한 판 더, 지면 복수전. 둘 다 위험합니다.`,
      `재미 담당 부서는 박수치고 수면 담당 부서는 항의합니다.`,
      `게임은 켜는 순간 쉽고 끄는 순간 어른의 영역입니다.`,
      `오늘의 멘탈은 ${name} 쪽 서버에 접속했습니다.`
    ],
    hobby: [
      `시작하면 기분은 풀리고 시간은 조용히 사라집니다.`,
      `${name} 쪽은 스트레스를 줄이고 시계를 놀라게 할 수 있습니다.`,
      `놀이는 짧게 하자고 시작해서 계획표를 흔듭니다.`,
      `재밌으면 됐고, 내일 피곤하면 그건 내일 회의입니다.`,
      `오늘 취미는 나를 달래고 알람을 긴장시킵니다.`,
      `하고 나면 "그래도 이 맛이지"가 나올 수 있습니다.`,
      `시간이 증발해도 기분이 살아나면 일단 본전입니다.`,
      `끝낼 타이밍만 잘 잡으면 오늘은 꽤 괜찮은 도피입니다.`,
      `오늘의 취미는 작은 휴가처럼 굴러갈 수 있습니다.`,
      `몸은 가만히 있어도 기분은 외출할 수 있습니다.`
    ],
    chore: [
      `${subject}는 하기 전만 제일 세고 끝나면 바로 조용해집니다.`,
      `미룬다고 사라지진 않고, 계속 눈에 밟히는 타입입니다.`,
      `끝내고 나면 집 공기가 "이제야?" 하고 말할 수 있습니다.`,
      `${name} 쪽으로 가면 귀찮음 대신 개운함이 남습니다.`,
      `지금 하면 미래의 내가 잔소리를 조금 줄입니다.`,
      `손대기 전엔 산 같고 끝나면 별거 아닌 척합니다.`,
      `오늘 처리하면 내일 같은 장면을 다시 안 봐도 됩니다.`,
      `미루면 편한 게 아니라 찝찝함을 예약하는 겁니다.`,
      `끝난 뒤 괜히 주변을 한 번 더 보게 되면 성공입니다.`,
      `하기 싫은 일은 시작 버튼이 제일 무겁습니다.`
    ],
    hygiene: [
      `물 닿기 전까지만 귀찮고, 닿고 나면 내가 맞습니다.`,
      `샤워기 소리가 오늘의 찝찝함을 퇴근시킵니다.`,
      `수건 잡는 순간 미래의 침대가 더 친절해집니다.`,
      `${name} 쪽은 5분 귀찮고 오래 개운합니다.`,
      `씻고 나면 "왜 이걸 미뤘지"가 거의 자동 재생됩니다.`,
      `침대 들어가기 전 몸이 먼저 고맙다고 할 수 있습니다.`,
      `머리는 핑계를 찾고 몸은 물을 기다립니다.`,
      `개운함은 작아 보여도 하루 끝에서는 꽤 큽니다.`,
      `욕실 문 닫는 순간 고민은 잠깐 밖에 남습니다.`,
      `귀찮음은 목소리가 크고, 개운함은 결과가 큽니다.`
    ],
    exercise: [
      `운동화가 장식품 신세를 벗으면 오늘은 이미 반 성공입니다.`,
      `시작 전엔 싫고 끝나면 괜히 당당해질 수 있습니다.`,
      `${name} 쪽은 몸을 힘들게 하고 자존감을 살립니다.`,
      `소파는 편하지만 운동 끝난 물 한 잔은 꽤 강합니다.`,
      `땀은 나고 핑계는 조금 줄어듭니다.`,
      `가기 전의 나와 끝난 뒤의 나는 서로 다른 사람일 수 있습니다.`,
      `오늘 움직이면 내일 변명이 하나 줄어듭니다.`,
      `운동은 하기 전이 제일 무섭고 끝나면 제법 착합니다.`,
      `몸은 투덜대도 거울 앞 표정은 조금 달라질 수 있습니다.`,
      `오늘 체력은 빠지고 자존감은 충전될 수 있습니다.`
    ],
    family: [
      `가족 방문은 거리보다 앉았을 때 공기가 먼저입니다.`,
      `${name} 쪽은 말투 하나, 눈치 하나가 결과를 바꿀 수 있습니다.`,
      `인사하고 앉는 순간 오늘의 난이도가 정해집니다.`,
      `편한 자리와 예의 있는 자리 사이에서 오늘은 ${name} 쪽입니다.`,
      `다녀오면 피곤해도 관계 온도계는 조금 움직입니다.`,
      `가족 앞에서는 선택보다 표정 관리가 더 큰 일입니다.`,
      `오늘은 누구를 보느냐보다 보고 난 뒤 마음이 중요합니다.`,
      `밥 한 끼가 그냥 밥 한 끼로 끝나지 않을 수 있습니다.`,
      `말은 짧아도 방문했다는 사실은 오래 갑니다.`,
      `가족 선택은 동선보다 마음의 숙제가 큽니다.`
    ],
    daily: [
      `미래의 내가 "그 정도면 됐다" 말고 조금 더 웃을 수 있습니다.`,
      `오늘은 거창한 정답보다 바로 넘길 수 있는 쪽입니다.`,
      `고민이 길어지면 선택보다 피로가 먼저 출근합니다.`,
      `${name} 쪽은 오늘 하루를 덜 버벅이게 만들 수 있습니다.`,
      `완벽한 답은 아니어도 지금 멈춰 있는 건 풀립니다.`,
      `선택지가 하나로 줄면 머릿속 소음도 조금 줄어듭니다.`,
      `오늘의 나는 일단 이쪽으로 핸들을 꺾었습니다.`,
      `나중에 웃길 수는 있어도 지금은 꽤 진심입니다.`,
      `작은 선택 하나가 하루의 로딩을 줄일 수 있습니다.`,
      `방금 고민 하나가 대기열에서 빠져나갔습니다.`,
      `오늘은 오래 재는 것보다 한 번 움직이는 쪽입니다.`,
      `정답지는 없지만 리모컨은 하나 골라야 합니다.`
    ]
  };
  return byCategory[category] || byCategory.daily;
}

function captureLinePool(category, winner, loser, question, signName) {
  const winnerName = escapeHtml(winner.name);
  const loserName = escapeHtml(loser.name);
  const winnerTopic = escapeHtml(withParticle(winner.name, "은", "는"));
  ...
  const common = [
    ...
    `고민은 길었고 결론은 생각보다 짧았습니다.`,
    `오늘은 완벽한 판단보다 덜 질척이는 선택입니다.`,
    `인생은 몰라도 오늘은 ${winnerName}입니다.`,
    `망설임이 길면 답보다 피곤함이 먼저 큽니다.`,
    `오늘의 나는 ${winnerName} 쪽으로 살짝 기울었습니다.`,
    `결정은 작아도 머릿속 소음은 줄어듭니다.`,
    `이 정도 고민이면 이제 하나 골라도 됩니다.`
  ];
  const pools = {
    food: [
      `${winnerTopic} 배를 설득하고, ${loserTopic} 미련을 남깁니다.`,
      `오늘 메뉴는 침샘이 먼저 결재했습니다.`,
      `먹고 나서 "아 이거였네" 하면 그걸로 끝입니다.`,
      `반대 메뉴는 다음 끼니로 보내면 됩니다.`,
      `오늘은 맛 평가보다 먹고 난 표정입니다.`,
      `배는 이미 알고 있었고 머리만 회의했습니다.`,
      `${winnerTopic} 앞에서는 다짐이 잠깐 자리를 비웁니다.`,
      `고민은 길었지만 숟가락은 솔직합니다.`,
      `오늘의 메뉴판은 ${winnerName}에서 접혔습니다.`,
      `먹고 조용해지는 쪽이면 거의 이긴 겁니다.`,
      `후회는 나중이고 지금은 ${winnerName} 냄새가 셉니다.`,
      `입은 ${winnerName}이라고 했고 자존심만 늦게 인정했습니다.`
    ],
    beverage: [
      `${winnerName}은 손에 잡히고, ${loserName}은 머리로 고민합니다.`,
      `오늘 한 모금은 기분 수리비입니다.`,
      `컵 안에 답이 있는 척해도 사실 손끝이 골랐습니다.`,
      `${winnerName} 한 모금 뒤 표정이 결과입니다.`,
      `오늘 카페인 회의는 ${winnerName}으로 종료됩니다.`,
      `커피는 취향보다 오늘 온도에 더 솔직합니다.`,
      `빨대 꽂는 순간 고민도 조금 작아집니다.`,
      `오늘은 음료가 아니라 나를 달래는 방식입니다.`
    ],
    drink: [
      `${winnerName}은 오늘 밤을 살리고, ${loserName}은 내일 아침을 살립니다.`,
      `한 잔만 한다는 말은 오늘도 신뢰를 잃었습니다.`,
      `술은 부르는데 알람은 아직 허락 안 했습니다.`,
      `폼보다 대화가 먼저 풀리는 쪽입니다.`,
      `${winnerName} 고르면 기분은 웃고 내일은 협상합니다.`,
      `오늘 잔은 ${winnerName} 쪽으로 먼저 기울었습니다.`,
      `안주는 핑계고 사실 분위기가 주문했습니다.`,
      `내일 물 찾을 각오까지 포함한 선택입니다.`
    ],
    shopping: [
      `버티기는 돈을 지키고, 구매는 정신건강을 지킵니다.`,
      `결제는 3초, 합리화는 3일입니다.`,
      `사지 않으면 잔고가 웃고, 사면 내가 웃습니다.`,
      `장바구니는 이미 결제된 척하고 있습니다.`,
      `${winnerName}은 욕망이고, ${loserName}은 예산표입니다.`,
      `지름신은 말이 많고 통장은 표정이 안 좋습니다.`,
      `배송조회까지 설레면 거의 졌습니다.`,
      `통장은 울어도 마음은 잠깐 조용해질 수 있습니다.`,
      `사고 자주 쓰면 승리, 안 쓰면 전시품입니다.`,
      `오늘의 소비는 물건보다 기분을 사는 쪽입니다.`
    ],
    relationship: [
      `보내는 건 1초, 해석은 3일입니다.`,
    ...
    travel: [
      `다녀오면 피곤해도 얘깃거리는 생깁니다.`,
      `오늘은 지도가 아니라 기분이 핀을 꽂았습니다.`,
      `${winnerName}은 발을 쓰고 마음을 환기합니다.`,
      `여행은 거리보다 돌아와서 먼저 꺼내는 말입니다.`,
      `사진첩보다 중요한 건 돌아오는 길 표정입니다.`,
      `짐은 귀찮고 도착지는 그 귀찮음을 변호합니다.`,
      `${winnerName} 고르면 하루가 평소보다 덜 복사본입니다.`,
      `밖에 나가면 피곤하고, 안 나가면 아쉽습니다.`,
      `오늘의 목적지는 ${winnerName}보다 바뀐 기분입니다.`,
      `여행은 완벽한 계획보다 나갔다는 증거가 큽니다.`,
      `차창 밖이 바뀌면 생각도 조금 바뀝니다.`,
      `오늘은 발보다 사진첩이 먼저 찬성했습니다.`
    ],
    place: [
      `나가면 피곤하고, 안 나가면 하루가 너무 조용합니다.`,
      `${winnerName} 고르면 체력은 빠지고 장면은 생깁니다.`,
      `신발 신는 순간 고민은 절반쯤 졌습니다.`,
      `집 밖 공기는 가끔 생각보다 일을 잘합니다.`,
      `오늘은 소파보다 바깥 공기가 한 표 앞섭니다.`,
      `돌아오면 피곤해도 "그래도 갔다"는 남습니다.`,
      `${winnerName}은 귀찮음을 지나 장면을 남깁니다.`,
      `안 나가면 편하고, 나가면 할 말이 생깁니다.`
    ],
    game: [
      ...
      `게임은 켜는 순간 쉽고 끄는 순간 어른의 영역입니다.`,
      `스트레스는 줄고 시계는 배신합니다.`,
      `오늘의 멘탈은 ${winnerName} 서버에 접속했습니다.`,
      `끝나고도 머릿속 미니맵이 꺼지지 않을 수 있습니다.`,
      `재미 담당은 웃고 수면 담당은 항의합니다.`
    ],
    hobby: [
      `${winnerName}은 스트레스를 줄이고 시계를 놀라게 합니다.`,
      `노는 건 시작보다 끝낼 타이밍이 어렵습니다.`,
      `재밌으면 됐고 피곤함은 내일 회의입니다.`,
      `오늘 취미는 작은 휴가처럼 굴러갑니다.`,
      `시간이 사라져도 기분이 살아나면 본전입니다.`,
      `${winnerName} 고르면 하루가 덜 무채색입니다.`,
      `놀이는 짧게 하자고 시작해서 계획표를 흔듭니다.`,
      `오늘은 생산성보다 숨통입니다.`
    ],
    chore: [
      `미루면 편한 게 아니라 찝찝함을 예약하는 겁니다.`,
      `${winnerName}은 귀찮고, 끝난 뒤 공기는 솔직합니다.`,
      `하기 싫은 일은 시작 버튼이 제일 무겁습니다.`,
      `오늘 처리하면 내일 같은 장면을 다시 안 봅니다.`,
      `집안일은 끝나면 갑자기 별거 아닌 척합니다.`,
      `손대기 전엔 산 같고 끝나면 바닥입니다.`,
      `${winnerName} 고르면 미래의 잔소리가 줄어듭니다.`,
      `귀찮음보다 눈에 밟히는 게 더 오래 갑니다.`
    ],
    hygiene: [
      ...
      `침대는 개운한 사람을 더 반깁니다.`,
      `찝찝함은 조용해도 존재감이 큽니다.`,
      `${winnerName}은 5분 귀찮고 오래 개운합니다.`,
      `수건 잡는 순간 미래의 내가 웃습니다.`,
      `씻고 나면 왜 미뤘는지부터 궁금해집니다.`
    ],
    exercise: [
      ...
      `운동 끝난 물 한 잔은 꽤 강합니다.`,
      `몸은 투덜대고 자존감은 충전됩니다.`,
      `오늘 움직이면 내일의 변명이 하나 줄어듭니다.`
    ],
    family: [
      `가족 선택은 동선보다 마음의 숙제가 큽니다.`,
      `가족 앞에서는 선택보다 표정 관리가 더 큰 일입니다.`,
      `${winnerName}은 편하거나 예의 있거나, 오늘 하나는 챙깁니다.`,
      `방문은 짧아도 다녀왔다는 사실은 오래 갑니다.`,
      `밥 한 끼가 그냥 밥 한 끼로 끝나지 않을 수 있습니다.`,
      `오늘은 누구를 보느냐보다 보고 난 뒤 마음입니다.`,
      `인사하고 앉는 순간 오늘 난이도가 정해집니다.`,
      `${winnerName} 쪽은 마음의 숙제를 조금 덜 밀립니다.`
    ],
    daily: common
  };
  const pool = (pools[category] || pools.daily).concat(common);
  if (category === "food" && includesAny(raw, ["과자", "스낵", "킥", "꽈배기", "꼬깔콘", "새우깡", "포카칩", "허니버터"])) {
    const snackLines = [
      `봉지 뜯는 순간 자제력은 휴가 갑니다.`,
      `과자는 가볍게 시작해서 손가락에 증거를 남깁니다.`,
      `조금만 먹는다는 말은 과자 앞에서 약합니다.`,
      `바닥 보이면 그때서야 오늘 선택을 인정합니다.`
    ];
    const categoryPool = pools[category] || pools.daily;
    return snackLines.concat(snackLines, snackLines, categoryPool);
  }
  const categoryPool = pools[category] || pools.daily;
  return categoryPool.concat(categoryPool, common);
}
...
function futureComment(category, winner, question, seed, sign) {
  ...
  if (category === "food") {
    if (includesAny(winnerRaw, ["킥", "꽈배기", "칩", "깡", "콘", "과자", "스낵", "허니", "포카", "새우", "바나나"])) {
      const snackFuture = [
        ...
        `봉지 접는 척하다가 한 번 더 열 수 있습니다.`,
        `입은 몰랐다고 하고 손은 계속 압니다.`,
        `과자 앞에서 자제력은 늘 임시직입니다.`,
        `조금만 먹는다는 말이 오늘도 과자에게 졌습니다.`,
        `부스러기가 남으면 그게 오늘의 서명입니다.`,
        `손에 묻은 가루가 선택의 사인입니다.`,
        `한 봉지의 무게가 갑자기 너무 가벼워질 수 있습니다.`,
        `소파에 앉으면 이 선택은 더 빨리 사라집니다.`,
        `옆에 음료가 있으면 게임은 이미 불리합니다.`,
        `봉지 바닥이 보이면 잠깐 인생을 돌아볼 수 있습니다.`
      ];
  ...
  const startsByCategory = {
    ...
    travel: [
      ...
      `가는 길 휴게소나 역 앞에서 이미 기분이 조금 바뀔 수 있습니다.`,
      `숙소 문 여는 순간 오늘 결정의 심사가 시작됩니다.`,
      `캐리어보다 마음이 먼저 현관을 나섰습니다.`,
      `돌아오는 길에 다리가 투덜대도 마음은 덜 납작할 수 있습니다.`,
      `지도 핀 하나가 하루 전체를 끌고 갈 예정입니다.`,
      `기차표든 주차권이든, 일단 일상 밖으로 한 칸 밀렸습니다.`,
      `날씨 앱을 세 번 보게 되면 이미 여행은 시작된 겁니다.`,
      `가서 먹을 첫 메뉴가 떠오르면 승산이 있습니다.`,
      `숙소 침대에 눕는 순간 오늘의 선택이 다시 평가됩니다.`,
      `계획표가 조금 삐끗해도 바람 맞으면 용서될 수 있습니다.`
    ],
    beverage: [
    ...
    daily: [
      ...
      `내일의 내가 이유를 물어도 대답은 할 수 있습니다.`,
      `작은 결정 하나가 머릿속 소음을 줄였습니다.`,
      `미래의 내가 한숨은 쉬어도 크게 화내진 않을 것 같습니다.`,
      `고민판이 조금 조용해졌습니다.`,
      `오늘의 나와 내일의 나가 서로 못 본 척 넘어갑니다.`,
      `대단한 답은 아니어도 오늘 굴러가게는 만들었습니다.`,
      `적어도 같은 고민을 30분 더 붙잡진 않아도 됩니다.`,
      `선택지가 더 커지기 전에 잡아둔 게 다행입니다.`,
      `오늘의 갈림길은 여기서 한 번 접어도 됩니다.`,
      `고민이 계속 커지기 전에 손잡이를 잡았습니다.`,
      `정답은 몰라도 방향은 하나 생겼습니다.`,
      `미래의 내가 "왜 그랬어?"보다 "그럴 수 있지"에 가깝습니다.`,
      `완벽한 판단은 아니어도 멈춰 있던 건 풀렸습니다.`,
      `머릿속 로딩 표시가 잠깐 사라졌습니다.`,
      `선택지가 둘에서 하나로 줄어든 것만으로도 오늘은 조금 가볍습니다.`
    ]
  };
  const extraStarts = memorableFutureCommentPool(category, winner, question, signName);
  const starts = (startsByCategory[category] || startsByCategory.daily).concat(extraStarts);
  if (category !== "daily") {
...
function shareableAdvice(category, winner, loser, question, seed, sign) {
  ...
  if (category === "exercise" && includesAny(`${question} ${winner.name} ${loser.name}`.replace(/\s/g, "").toLowerCase(), ["산책", "런닝", "러닝", "공원"])) {
    const exerciseLines = [
      `${winnerName}은 몸을 깨우고, ${loserName}은 다른 속도로 설득합니다.`,
      `오늘 운동은 세게 하는 것보다 끝까지 하는 쪽이 이깁니다.`,
      `${winnerName} 쪽은 땀보다 리듬을 먼저 챙기는 선택입니다.`,
      `운동화가 무리 말고 꾸준함 쪽으로 끈을 묶었습니다.`,
      `${winnerName} 고르면 몸은 움직이고 핑계는 조금 줄어듭니다.`
    ];
    return withZodiacTag(pick(exerciseLines, hashText(`${question}-${winner.name}-${loser.name}-${signName}-${seed}-exercise-share`)));
  }
  const pools = {
    food: [
      `${winnerTopic} 배를 설득하고, ${loserTopic} 미련을 남깁니다.`,
    ...
    travel: [
      `${winnerName}은 도착 전부터 하루 리듬이 먼저 바뀝니다.`,
      `여행은 멀리보다 돌아오는 길 표정이 진짜입니다.`,
      `${loserName}도 좋지만 오늘 동선은 ${winnerName} 쪽이 덜 억지스럽습니다.`,
      `${winnerName} 고르면 몸은 피곤해도 일상은 잠깐 밀립니다.`,
      `지도 앱이 먼저 웃는 쪽은 ${winnerName}입니다.`,
      `${winnerName}은 가는 길부터 기분 전환이 시작됩니다.`,
    ...
    amusement: [
      ...
      `${winnerName}은 하루를 이벤트로 만들고 체력에게 영수증을 보냅니다.`,
      `재미가 이기면 발바닥은 조용히 항소합니다.`,
      `${winnerName} 쪽은 집 가는 길 말수가 줄어도 할 말은 남습니다.`
    ],
    hobby: [
      `${winnerName}은 재미를 주고, 시간은 몰래 가져갑니다.`,
      ...
      `${winnerName}은 시작 버튼이 쉽고 종료 버튼이 어렵습니다.`,
      `놀이는 짧게 하자고 시작해서 시계를 놀라게 합니다.`,
      `${winnerName} 고르면 기분은 풀리고 시간은 증발합니다.`
    ],
    hygiene: [
      `씻기 전엔 귀찮고, 씻고 나면 내가 맞습니다.`,
      `물 틀기 전까지만 인생 최대 고민입니다.`,
  ...
  const extraPool = captureLinePool(category, winner, loser, question, signName);
  const basePool = (pools[category] || pools.daily).concat(extraPool);
...
function categoryFramePool(category, question = "") {
  ...
  const byCategory = {
    money: ["stability_vs_chance", "volatility", "sleep_quality", "cash_position", "timing"],
    shopping: ["price_vs_desire", "daily_use", "newness_regret", "repair_or_replace", "wallet_aftertaste"],
    food: ["temperature", "craving", "heaviness", "clean_finish", "snack_hand"],
    beverage: ["temperature", "wake_up", "comfort_cup", "aftertaste"],
    ...
    chore: ["visible_mess", "future_me", "start_barrier", "after_clean_air"],
    hygiene: ["freshness", "sleep_after", "start_barrier", "body_signal"],
    exercise: ["start_barrier", "after_sweat", "body_condition", "tomorrow_pride"],
    study: ["deadline_pressure", "small_start", "future_debt", "focus_energy"],
    game: ["fun_now", "time_leak", "team_stress", "one_more_round"],
    hobby: ["fun_now", "time_leak", "energy_after", "social_noise"],
    pet: ["daily_routine", "affection_style", "space_care", "long_term"],
    amusement: ["ride_density", "waiting_line", "stamina_bill", "photo_memory"],
    daily: ["future_regret", "effort_now", "mind_noise", "simple_next_step"]
  };
  const pool = (byCategory[category] || byCategory.daily).slice();
  if (includesAny(raw, ["적금", "주식", "코인", "매수", "매도", "투자", "팔까", "살까"])) pool.push("stability_vs_chance", "volatility", "timing");
...
function selectCategoryFrame(category, question, seed, sign) {
  const pool = categoryFramePool(category, question);
  return pick(pool, hashText(`${question}-${category}-${seed}-${sign && sign[0] ? sign[0] : ""}-category-frame`));
}

function categoryFrameKeywords(frame) {
  const map = {
    stability_vs_chance: ["안정", "적금", "관망", "보유", "기다", "현금", "안심", "기회", "주식", "코인", "상승"],
    volatility: ["코인", "변동", "리스크", "손실", "급등", "급락", "주식"],
    sleep_quality: ["안심", "관망", "보유", "기다", "편안", "부담"],
    cash_position: ["현금", "적금", "아끼", "돈절약", "예산", "통장"],
    timing: ["지금", "타이밍", "오늘", "기다", "매수", "매도", "판다"],
    price_vs_desire: ["가격", "비용", "중고", "실속", "욕망", "새거", "새것", "지름", "만족"],
    daily_use: ["매일", "자주", "사용", "화질", "성능", "렉", "편의", "오래"],
    newness_regret: ["신제품", "새모델", "새", "기다", "후회", "출시"],
    repair_or_replace: ["고장", "수리", "바꾼다", "교체", "버틴다", "멀쩡"],
    wallet_aftertaste: ["통장", "결제", "할부", "비싸", "잔고"],
    temperature: ["차갑", "시원", "뜨거", "따뜻", "냉면", "콩국수", "아이스", "라떼"],
    craving: ["매콤", "달달", "바삭", "냄새", "고기", "라면", "치킨", "떡볶이", "짬뽕"],
    heaviness: ["든든", "푸짐", "묵직", "고기", "한우", "삼겹살", "감자탕", "밥"],
    clean_finish: ["깔끔", "가볍", "담백", "김밥", "냉면", "초밥", "사이다"],
    snack_hand: ["과자", "칩", "콘", "깡", "꽈배기", "킥", "손", "봉지"],
    wake_up: ["아아", "아이스", "카페인", "깨", "졸려"],
    comfort_cup: ["라떼", "따뜻", "위로", "달달", "부드"],
    aftertaste: ["깔끔", "뒷맛", "느끼", "사이다", "아메리카노"],
    tonight_speed: ["소주", "맥주", "잔", "빠르", "말", "술"],
    tomorrow_face: ["내일", "숙취", "알람", "물", "얼굴"],
    people_mood: ["친구", "대화", "사람", "분위기", "자리"],
    stop_timing: ["한잔", "멈", "막잔", "귀가", "시간"],
    rest_vs_activity: ["리조트", "휴식", "제주", "활동", "부산", "캠핑", "체험"],
    travel_cost: ["비용", "비행", "숙소", "예산", "교통"],
    memory_density: ["사진", "추억", "바다", "카페", "야경", "먹거리", "이야기"],
    movement_fatigue: ["이동", "피곤", "거리", "걷", "짐", "동선"],
    food_route: ["맛집", "카페", "먹거리", "회", "국밥", "시장"],
    movement_cost: ["이동", "택시", "버스", "거리", "시간", "체력"],
    scene_gain: ["장면", "사진", "바깥", "공기", "구경"],
    crowd_energy: ["사람", "붐", "시끄", "조용", "대기"],
    return_home: ["돌아", "귀가", "집", "피곤", "만족"],
    send_aftershock: ["연락", "카톡", "전화", "답장", "읽음"],
    ...
    after_visit_mind: ["마음", "부담", "편안", "숙제"],
    first_reaction: ["우와", "반응", "표정", "포장", "선물"],
    long_play: ["오래", "반복", "장난감", "사용", "가지고"],
    price_smile: ["가격", "예산", "만족", "영수증"],
    receiver_routine: ["아이", "아들", "조카", "받는", "루틴"],
    child_reaction: ["아이", "웃", "뛰", "놀이", "반응"],
    parent_stamina: ["보호자", "부모", "체력", "허리", "피곤"],
    weather_backup: ["비", "날씨", "실내", "미세먼지", "춥", "덥"],
    safety_space: ["안전", "부딪", "공간", "실내", "넓"],
    visible_mess: ["먼지", "물때", "그릇", "싱크대", "차", "욕실", "화장실"],
    future_me: ["내일", "나중", "미래", "미루", "오늘"],
    start_barrier: ["귀찮", "시작", "일어나", "물", "손"],
    after_clean_air: ["개운", "깨끗", "광택", "공기", "정리"],
    freshness: ["개운", "씻", "샤워", "물", "찝찝"],
    sleep_after: ["잠", "침대", "이불", "수면"],
    body_signal: ["몸", "피곤", "땀", "냄새"],
    after_sweat: ["땀", "운동", "물", "뿌듯"],
    body_condition: ["몸", "아파", "무리", "체력"],
    tomorrow_pride: ["내일", "자존감", "뿌듯", "변명"],
    deadline_pressure: ["시험", "마감", "과제", "숙제", "내일"],
    small_start: ["20분", "시작", "한페이지", "책상"],
    future_debt: ["미루", "부담", "빚", "압박"],
    focus_energy: ["집중", "공부", "정리", "진도"],
    fun_now: ["재미", "게임", "놀", "취미", "스트레스"],
    time_leak: ["시간", "한판", "새벽", "플레이", "삭제"],
    team_stress: ["팀", "랭크", "채팅", "친구"],
    one_more_round: ["한판", "복수", "다시", "라운드"],
    energy_after: ["기분", "피곤", "충전", "풀림"],
    social_noise: ["친구", "사람", "대화", "소리"],
    daily_routine: ["산책", "밥", "모래", "털", "루틴"],
    affection_style: ["강아지", "고양이", "애교", "독립", "반응"],
    space_care: ["집", "공간", "청소", "책임"],
    long_term: ["매일", "오래", "책임", "생활"],
    ride_density: ["놀이기구", "대기", "하루", "롯데월드", "서울랜드"],
    waiting_line: ["줄", "대기", "사람", "기다"],
    stamina_bill: ["체력", "다리", "피곤", "발"],
    photo_memory: ["사진", "추억", "인생샷", "하루"],
    effort_now: ["지금", "귀찮", "노력", "시작"],
    mind_noise: ["마음", "생각", "소음", "고민"],
    simple_next_step: ["바로", "간단", "다음", "작은"]
  };
  return map[frame] || [];
}

function categoryFrameOptionBoost(category, question, analysis, seed, sign) {
  const frame = selectCategoryFrame(category, question, seed, sign);
  const compact = `${analysis.name || ""} ${analysis.subjectName || ""} ${(analysis.features || []).join(" ")} ${analysis.vibe || ""}`.replace(/\s/g, "").toLowerCase();
  const keywords = categoryFrameKeywords(frame);
  let boost = keywords.some((word) => compact.includes(String(word).replace(/\s/g, "").toLowerCase())) ? 9 : 0;
  if (analysis.intent === "skip" && ["sleep_quality", "cash_position", "wallet_aftertaste", "future_me", "body_condition", "tomorrow_face"].includes(frame)) boost += 5;
  if (analysis.intent === "go" && ["craving", "fun_now", "scene_gain", "memory_density", "first_reaction", "after_clean_air", "after_sweat"].includes(frame)) boost += 5;
  if (category === "food" && frame === "temperature" && includesAny(compact, ["냉면", "콩국수", "아이스", "따뜻", "라떼", "국물"])) boost += 6;
...
function categoryFrameReasonLine(frame, category, winner, loser, question) {
  const winnerName = escapeHtml(winner.name);
  const loserName = escapeHtml(loser.name);
  const winnerTopic = escapeHtml(withParticle(winner.name, "은", "는"));
  const loserTopic = escapeHtml(withParticle(loser.name, "은", "는"));
  const f0 = escapeHtml((winner.features || [])[0] || "오늘 바로 떠오르는 장점");
  const l0 = escapeHtml((loser.features || [])[0] || "다른 장점");
  const lines = {
    stability_vs_chance: `${winnerTopic} 오늘 돈을 불리는 상상보다 흔들려도 버틸 수 있는지가 먼저 보입니다. ${loserTopic} 기회는 있지만 마음의 진폭이 더 큽니다.`,
    volatility: `${winnerTopic} 오르내림까지 감당하는 선택입니다. ${loserName}보다 심장은 더 뛰지만, 오늘 기준은 변동성을 어떻게 볼지예요.`,
    sleep_quality: `${winnerTopic} 고르고 나서 잠이 덜 깨질 쪽입니다. ${loserTopic} 숫자는 재밌어도 밤에 차트를 다시 열게 만들 수 있습니다.`,
    cash_position: `${winnerTopic} 돈을 움직이는 쪽보다 지키는 쪽의 힘이 있습니다. 오늘은 수익보다 손에 남는 감각을 봅니다.`,
    price_vs_desire: `${winnerTopic} 가격표와 욕망이 정면으로 만나는 선택입니다. ${loserName}도 끌리지만 지금은 사고 나서 자주 웃을지가 더 큽니다.`,
    daily_use: `${winnerTopic} 한 번의 설렘보다 매일 손이 가는지가 포인트입니다. ${loserName}보다 실제로 쓰는 장면이 더 자주 보입니다.`,
    newness_regret: `${winnerTopic} 지금 갖는 만족과 나중에 새 모델을 보는 마음 사이의 선택입니다. 오늘은 기다림보다 쓰는 장면이 더 앞섭니다.`,
    repair_or_replace: `${winnerTopic} 고장과 불편을 계속 참을지, 한 번에 갈아엎을지의 문제입니다. 오늘은 불편을 얼마나 더 볼 수 있느냐가 핵심입니다.`,
    wallet_aftertaste: `${winnerTopic} 결제 직후 표정까지 봐야 합니다. ${loserName}보다 통장이 덜 째려보는지, 내가 더 웃는지가 갈립니다.`,
    temperature: `${winnerTopic} 온도부터 다릅니다. ${loserName}도 좋지만 오늘 입은 시원함/따뜻함 중 한쪽에 먼저 반응했습니다.`,
    craving: `${winnerTopic} 설명보다 당김이 먼저 온 선택입니다. ${loserName}도 후보지만 냄새와 식감이 오늘은 이쪽을 더 세게 부릅니다.`,
    heaviness: `${winnerTopic} 가볍게 지나가는 선택이 아니라 배를 확실히 채우는 쪽입니다. ${loserName}보다 든든함이 앞에 섰습니다.`,
    clean_finish: `${winnerTopic} 먹고 나서 깔끔하게 끝나는 그림이 있습니다. ${loserName}보다 뒤끝이 덜 무거운 쪽이에요.`,
    snack_hand: `${winnerTopic} 손이 한 번 들어가면 계속 가는 타입입니다. ${loserName}도 좋지만 오늘 과자판은 손맛이 먼저입니다.`,
    wake_up: `${winnerTopic} 기분보다 눈꺼풀을 먼저 깨우는 선택입니다. ${loserName}보다 지금 필요한 건 각성 쪽입니다.`,
    comfort_cup: `${winnerTopic} 정신 차리기보다 나를 달래는 컵에 가깝습니다. 오늘은 효율보다 위로가 먼저 보입니다.`,
    tonight_speed: `${winnerTopic} 잔이 돌기 시작하면 대화가 빨리 풀리는 쪽입니다. ${loserName}보다 오늘 밤 속도가 빠릅니다.`,
    tomorrow_face: `${winnerTopic} 오늘 밤보다 내일 아침 얼굴까지 같이 보는 선택입니다. ${loserTopic}은 지금은 좋아도 알람 앞에서 말이 달라질 수 있습니다.`,
    rest_vs_activity: `${winnerTopic} 쉬는 여행인지 움직이는 여행인지부터 갈립니다. 오늘은 ${loserName}보다 몸이 받아들일 하루 모양이 더 중요합니다.`,
    travel_cost: `${winnerTopic} 숙소, 교통, 이동 시간을 같이 끌고 옵니다. ${loserName}보다 비용 대비 기분 전환이 더 잘 보입니다.`,
    memory_density: `${winnerTopic} 다녀와서 꺼낼 사진과 얘깃거리가 먼저 보입니다. ${loserName}도 좋지만 남는 장면은 이쪽이 더 많습니다.`,
    ...
    daily_routine: `${winnerTopic} 귀여움보다 매일 반복되는 생활까지 보는 선택입니다. ${loserName}보다 루틴이 더 선명합니다.`,
    ride_density: `${winnerTopic} 하루 안에 얼마나 꽉 차게 노느냐가 핵심입니다. ${loserName}보다 이벤트 밀도가 더 큽니다.`,
    effort_now: `${winnerTopic} 지금 조금 움직이면 뒤가 가벼워지는 선택입니다. ${loserTopic}은 편하지만 같은 고민을 남길 수 있습니다.`,
    mind_noise: `${winnerTopic} 마음속 소음을 줄이는 쪽입니다. ${loserName}도 이유는 있지만 생각이 더 오래 남을 수 있습니다.`,
    simple_next_step: `${winnerTopic} 복잡한 정답보다 바로 다음 행동이 보입니다. ${loserName}보다 오늘 굴러가기 쉽습니다.`
  };
  return cleanPlayTone(lines[frame] || `${winnerTopic} ${f0} 쪽이 먼저 보입니다. ${loserTopic} ${l0}도 있지만 오늘 기준은 ${winnerName} 쪽으로 기울었습니다.`);
}

function decisionLensPool(category, question = "") {
  const raw = String(question || "").replace(/\s/g, "").toLowerCase();
  const base = ["future_regret", "emotion_satisfaction", "fun", "laziness"];
  const byCategory = {
    food: ["emotion_satisfaction", "fun", "laziness", "future_regret"],
    beverage: ["emotion_satisfaction", "efficiency", "laziness"],
    drink: ["fun", "risk", "future_regret", "relationship"],
    travel: ["memory", "stamina", "cost", "fun", "future_regret"],
    place: ["stamina", "fun", "laziness", "memory"],
    childcare: ["stamina", "risk", "memory", "efficiency"],
    shopping: ["cost", "efficiency", "future_regret", "risk"],
    money: ["risk", "cost", "future_regret", "efficiency"],
    relationship: ["relationship", "future_regret", "emotion_satisfaction", "risk"],
    family: ["relationship", "future_regret", "emotion_satisfaction", "efficiency"],
    chore: ["laziness", "efficiency", "future_regret", "stamina"],
    hygiene: ["laziness", "efficiency", "emotion_satisfaction"],
    exercise: ["stamina", "future_regret", "efficiency"],
    study: ["efficiency", "future_regret", "risk"],
    game: ["fun", "risk", "future_regret"],
    hobby: ["fun", "stamina", "future_regret"],
    pet: ["relationship", "risk", "future_regret"],
    amusement: ["fun", "stamina", "memory"],
    gift: ["relationship", "emotion_satisfaction", "memory"]
  };
  const pool = (byCategory[category] || base).slice();
  if (includesAny(raw, ["돈", "비싸", "가격", "결제", "통장", "할부"])) pool.push("cost", "cost");
  if (includesAny(raw, ["피곤", "체력", "힘들", "아프", "졸려", "귀찮"])) pool.push("stamina", "laziness");
  if (includesAny(raw, ["위험", "불안", "손해", "후폭풍", "걱정", "실패"])) pool.push("risk", "future_regret");
  if (includesAny(raw, ["친구", "가족", "연인", "상대", "부모", "처가", "본가"])) pool.push("relationship");
  if (includesAny(raw, ["추억", "사진", "여행", "기억", "선물"])) pool.push("memory");
  if (includesAny(raw, ["재밌", "놀", "게임", "신나", "웃"])) pool.push("fun");
  return uniqueList(pool);
}

function selectDecisionLenses(category, question, seed, sign) {
  const pool = decisionLensPool(category, question);
  const first = pick(pool, hashText(`${question}-${category}-${seed}-${sign && sign[0] ? sign[0] : ""}-lens-1`));
  const rest = pool.filter((item) => item !== first);
  const shouldUseTwo = rest.length && (hashText(`${question}-${category}-${seed}-lens-count`) % 100) >= 42;
  const second = shouldUseTwo ? pick(rest, hashText(`${question}-${category}-${seed}-${first}-lens-2`)) : null;
  return second ? [first, second] : [first];
}

function lensOptionBoost(lenses, analysis, category, question) {
  const text = `${analysis.name || ""} ${analysis.subjectName || ""} ${(analysis.features || []).join(" ")}`.replace(/\s/g, "").toLowerCase();
  const q = String(question || "").replace(/\s/g, "").toLowerCase();
  let boost = 0;
  lenses.forEach((lens) => {
    if (lens === "future_regret") {
      if (analysis.intent === "go" && includesAny(q, ["해야", "마감", "출근", "청소", "설거지", "연락", "사과"])) boost += 6;
      if (analysis.intent === "skip" && includesAny(q, ["피곤", "늦", "무리", "비싸", "위험"])) boost += 5;
      if (includesAny(text, ["후회", "안심", "오래", "실패적", "실패 적", "부담없", "부담 없음"])) boost += 4;
    }
    if (lens === "efficiency") {
      if (includesAny(text, ["빠른", "간단", "실속", "효율", "동선", "정리", "시간", "바로", "사용빈도", "사용 빈도"])) boost += 7;
      if (analysis.intent === "go" && ["chore", "study", "hygiene", "shopping"].includes(category)) boost += 4;
    }
    if (lens === "emotion_satisfaction") {
      if (includesAny(text, ["만족", "달달", "위로", "편안", "기분", "표정", "따뜻", "맛", "소확행"])) boost += 7;
      if (["food", "beverage", "gift"].includes(category)) boost += 3;
    }
    if (lens === "cost") {
      if (analysis.intent === "skip" || includesAny(text, ["돈절약", "돈 절약", "부담없", "부담 없음", "실속", "무료", "비용", "예산"])) boost += 8;
      if (includesAny(text, ["비용", "입장료", "가격", "결제", "통장"])) boost -= 2;
    }
    if (lens === "stamina") {
      if (analysis.intent === "skip" && includesAny(q, ["피곤", "늦", "무리", "힘들"])) boost += 7;
      if (includesAny(text, ["편안", "휴식", "덜지", "덜 지", "앉", "실내", "회복", "동선"])) boost += 6;
      if (includesAny(text, ["체력소모", "체력 소모", "많이걷", "많이 걷", "피곤", "대기줄"])) boost -= 3;
    }
    if (lens === "relationship") {
      if (includesAny(text, ["관계", "배려", "마음", "상대", "가족", "예의", "반응", "표정", "말"])) boost += 8;
      if (["relationship", "family", "gift", "pet"].includes(category)) boost += 3;
    }
    if (lens === "risk") {
      if (analysis.intent === "skip" && includesAny(q, ["위험", "불안", "손해", "후폭풍", "비싸", "무리"])) boost += 8;
      if (includesAny(text, ["안정", "리스크", "관망", "기다", "실패", "부담"])) boost += 5;
    }
    if (lens === "fun") {
      if (includesAny(text, ["재미", "신나", "놀이", "게임", "웃", "자극", "모험", "새로", "호기심"])) boost += 8;
      if (["game", "hobby", "amusement", "food"].includes(category)) boost += 3;
    }
    if (lens === "laziness") {
      if (analysis.intent === "skip" && includesAny(q, ["귀찮", "피곤", "나중", "내일"])) boost += 7;
      if (includesAny(text, ["간단", "편", "실내", "바로", "부담없", "부담 없음", "쉬운"])) boost += 6;
    }
    if (lens === "memory") {
      if (includesAny(text, ["사진", "추억", "기억", "여행", "선물", "바다", "풍경", "경험", "이야기"])) boost += 8;
      if (["travel", "gift", "amusement", "childcare"].includes(category)) boost += 3;
    }
  });
  return boost;
}

function decisionLensReasonLine(lenses, winner, loser, category, question, seed) {
  if (!lenses || !lenses.length) return "";
  const lens = lenses[0];
  const second = lenses[1] || "";
  const winnerName = escapeHtml(winner.name);
  const loserName = escapeHtml(loser.name);
  const winnerSubject = withParticle(winner.name, "은", "는");
  const loserSubject = withParticle(loser.name, "은", "는");
  const f0Raw = (winner.features || [])[0] || "오늘 바로 떠오르는 장점";
  const f0 = escapeHtml(f0Raw);
  const f0Subject = withParticle(f0Raw, "은", "는");
  const f0Ga = withParticle(f0Raw, "이", "가");
  const f1 = escapeHtml((winner.features || [])[1] || "선택 뒤 느낌");
  const l0 = escapeHtml((loser.features || [])[0] || "다른 장점");
  const extra = second ? {
    future_regret: "후회까지 같이 보니까",
    efficiency: "시간 계산까지 넣으니까",
    emotion_satisfaction: "기분값까지 얹으니까",
    cost: "지갑 표정까지 보니까",
    stamina: "체력까지 끼워 넣으니까",
    relationship: "사람 마음까지 같이 보니까",
    risk: "불안 요소까지 세어보니까",
    fun: "재미 점수까지 넣으니까",
    laziness: "귀찮음 비용까지 더하니까",
    memory: "나중에 남을 장면까지 보니까"
  }[second] || "다른 기준까지 같이 보니까" : "";
  const lines = {
    future_regret: [
      `오늘은 "나중에 덜 후회할 쪽" 렌즈로 봤습니다. ${winnerSubject} ${f0Ga} 먼저 보이고, ${loserSubject} ${l0} 쪽이라 ${extra || "결론이 조금 더 또렷해집니다"}.`,
      `지금 끌리는 것보다 끝나고 나를 덜 괴롭힐 쪽을 봤습니다. 그 기준에선 ${winnerName} 쪽이 한 발 앞입니다.`
    ],
    efficiency: [
      `오늘은 효율로 봤습니다. ${winnerSubject} ${f0}, ${f1} 쪽이라 고민 시간을 길게 잡아먹지 않습니다.`,
      `시간과 동선을 먼저 놓고 보면 ${winnerName} 쪽이 덜 꼬입니다. ${loserName}도 좋지만 계산이 조금 더 붙습니다.`
    ],
    emotion_satisfaction: [
      `오늘은 마음이 바로 좋아지는 쪽을 봤습니다. ${winnerSubject} ${f0Ga} 빨리 오고, ${loserSubject} 조금 더 머리로 따지는 선택입니다.`,
      `기분값을 넣으면 ${winnerName} 쪽이 앞섭니다. 이유는 거창하지 않고, 선택하는 순간 표정이 조금 더 풀리는 쪽이라서요.`
    ],
    cost: [
      `오늘은 지갑 눈치를 봤습니다. ${winnerSubject} 비용 대비 ${f0Ga} 남고, ${loserSubject} 마음은 가도 계산서가 한 번 더 말을 겁니다.`,
      `돈 쓰고도 웃을 수 있느냐로 보면 ${winnerName} 쪽입니다. 아끼는 쪽이든 쓰는 쪽이든 오늘은 찝찝함이 덜한 쪽을 골랐습니다.`
    ],
    stamina: [
      `오늘은 체력 기준입니다. ${winnerSubject} ${f0Ga} 있지만 몸이 감당할 그림이 더 잘 나오고, ${loserSubject} 끝나고 다리가 항의할 수 있습니다.`,
      `하고 나서 몸이 덜 삐지는 쪽을 봤습니다. 그 렌즈에선 ${winnerName} 쪽이 조금 더 현실적입니다.`
    ],
    relationship: [
      `오늘은 사람 마음을 먼저 봤습니다. ${winnerSubject} ${f0Ga} 있고, ${loserSubject} ${l0} 쪽이라 관계 온도까지 보면 이쪽이 덜 어색합니다.`,
      `혼자만 좋은 선택보다 서로 덜 불편한 쪽을 봤습니다. 그래서 ${winnerName} 쪽이 조금 더 말이 됩니다.`
    ],
    risk: [
      `오늘은 리스크를 먼저 봤습니다. ${winnerSubject} ${f0Ga} 있고, ${loserSubject} 나중에 변수가 더 붙을 수 있습니다.`,
      `잘되면 좋은 쪽보다 망했을 때 덜 아픈 쪽을 봤습니다. 그 기준에선 ${winnerName} 쪽이 안전합니다.`
    ],
    fun: [
      `오늘은 재미 점수를 크게 봤습니다. ${winnerSubject} ${f0Ga} 바로 떠오르고, ${loserSubject} 조금 더 얌전한 후보입니다.`,
      `정답보다 "하고 나서 웃을 수 있냐"로 보면 ${winnerName} 쪽입니다. 오늘은 재미가 논리를 살짝 이겼습니다.`
    ],
    laziness: [
      `오늘은 귀찮음 비용을 계산했습니다. ${winnerSubject} ${f0} 쪽이라 시작 장벽이 낮고, ${loserSubject} 손이 한 번 더 갑니다.`,
      `지금 내가 얼마나 움직이기 싫은지도 판단에 넣었습니다. 그럼 ${winnerName} 쪽이 덜 버겁습니다.`
    ],
    memory: [
      `오늘은 나중에 남는 장면을 봤습니다. ${winnerSubject} ${f0}, ${f1} 쪽이라 기억에 남을 가능성이 큽니다.`,
      `그날이 지나고도 말할 게 남느냐로 보면 ${winnerName} 쪽입니다. ${loserName}도 좋지만 오늘 사진은 이쪽이 더 잘 찍힙니다.`
    ]
  };
  return cleanPlayTone(pick(lines[lens] || lines.future_regret, hashText(`${question}-${winner.name}-${loser.name}-${category}-${seed}-${lenses.join("-")}-lens-line`)));
}

function scoreOption(analysis, category, question, mood, seed, sign, cards = [], decisionLenses = []) {
  ...
  score += lensOptionBoost(decisionLenses, analysis, category, question);
  score += categoryFrameOptionBoost(category, question, analysis, seed, sign);
